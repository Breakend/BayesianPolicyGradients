...
class BayesianDiagGaussianPdType(PdType):
    def __init__(self, size):
        self.size = size
    def pdclass(self):
        return BayesianGaussianPd
    def param_shape(self):
        return [2*self.size]
    def sample_shape(self):
        return [self.size]
    def sample_dtype(self):
        return tf.float32
    def pdfromflat(self, flat, tau):
        return self.pdclass()(flat, tau)
...
class BayesianGaussianPd(Pd):
    def __init__(self, flat, tau, alpha=.5):
        """
        Flat here really isn't flat it's a list of mc dropout networks
        """
        self.flat = flat
        mean, var = U.bayesian_block(flat)
        #http://mlg.eng.cam.ac.uk/yarin/blog_3d801aa532c1ce.html
        self.tau = tau
        # lengthscale**2.0 * (keep_prob) / (2.0 * flat[0].get_shape().as_list()[-1] * l2_reg)
        print("Tau precision %f" % self.tau)
        var += self.tau**-1.0

        logstd = tf.log(tf.sqrt(var))
        self.mean = mean
        self.logstd = logstd
        self.alpha = alpha
    ...
    def neglogp(self, x):
        ...
        sumsq *= (-.5 * self.alpha * self.tau)
        # should log
        loss = (-1.0 * self.alpha ** -1.0) * logsumexp(sumsq, 0) + 0.5 * tf.to_float(tf.shape(x)[-1]) * tf.log(self.tau)
        # something *= logsumexp(sumsq, 0) #+ 0.5 * tf.to_float(tf.shape(x)[-1]) * tf.log(self.tau)
        return loss

    def kl(self, other):
        assert isinstance(other, BayesianGaussianPd)
    ...
    def sample(self):
        probs = [[1.0/float(len(self.flat))] * len(self.flat)]
        flat_tensor = tf.convert_to_tensor(self.flat)
        samples = tf.multinomial(tf.log(probs), 1) # note log-prob
        random_dropout_net = flat_tensor[tf.cast(samples[0][0], tf.int32)]
        return random_dropout_net
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
...
def make_pdtype(ac_space, bayes=False):
    from gym import spaces
    if isinstance(ac_space, spaces.Box) and bayes:
        assert len(ac_space.shape) == 1
        return BayesianDiagGaussianPdType(ac_space.shape[0])
    elif isinstance(ac_space, spaces.Box) and not bayes:
