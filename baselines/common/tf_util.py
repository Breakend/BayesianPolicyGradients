...
def variable_scope(name, absolute=None):
  """
...
def bayes_dropout(x, keep_prob, noise_shape=None, seed=None, name=None):  # pylint: disable=invalid-name
  """
  Generates a single dropout mask and applies it.
  """
  keep_prob_orig = keep_prob
  global num_dropout_nets
  with tf.variable_scope("dropout" + str(num_dropout_nets)) as name:
    # TODO: this is a super hack to deal with variable scope reuse.
    num_dropout_nets += 1
    ...
    binary_tensor = tf.Variable(np_random_mask, trainable=False, dtype=x.dtype)
